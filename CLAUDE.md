...
| `web/dist/` | **Generated** by `bun run build`, committed (tracked) so git clone/update carries it. ComfyUI serves it at `/extensions/comfyui-filename-prefix/`. |
...
- **Pack directory name is part of the URL.** `web/dist/index.js` is served at
  `/extensions/comfyui-filename-prefix/index.js`. Renaming the pack dir breaks every fetch. If
...
```sh
curl -s -o /dev/null -w "%{http_code}\n" http://127.0.0.1:8188/extensions/comfyui-filename-prefix/index.js
```
